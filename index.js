...
(async () => {
    ...
    app.post('/bulk', (req, res) => {
        ...
        const job = new Job(req, res, true);
        for (const data of req.body.links) {
            const link = new InspectURL(data.link);
            if (!link.valid) return errors.InvalidInspect.respond(res);
            let price;
    ...
    queue.process(CONFIG.logins.length, botController, async (job) => {
        ...
        if (postgres) await postgres.insertItemData(itemData.iteminfo, job.data.price);

        itemData.iteminfo = Object.assign(itemData.iteminfo, await (postgres ? postgres.getItemRank(itemData.iteminfo.a) : {}));
